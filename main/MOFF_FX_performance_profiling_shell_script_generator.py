...
xmax = [40.0, 160.0, None, None, None, None]
ymax = [40.0, 160.0, None, None, None, None]
...
array_layout = ['MWA-128T', 'MWA-128T', 'HEX-19', 'HEX-37', 'HEX-61', 'HEX-91']
layout_fraction = [1.0]
antenna_size = [4.4, 4.4, 14.0, 14.0, 14.0, 14.0]
...
for ind, layout in enumerate(array_layout):
    ...
    for rnum in range(nruns):
        ...
        print n_antennas, n_antennas*(n_antennas-1)/2
        for ti in nts:
            ants = []
            aar = AA.AntennaArray()
            ...
            for itr in niter:
                if layout.split('-')[0] == 'MWA':
                    fname = '/data3/t_nithyanandan/project_MOFF/simulated/MWA/profiling/FX_serial_all_lines/MOFF_FX_performance_comparison_{0}_{1:.1f}mx{2:.1f}m_{3:0d}x{4:.1f}_kHz_{5:.1f}_MHz_{6:0d}_ant_{7:0d}_acc_{8:0d}_pix_r{9:0d}'.format(layout,ant_sizex,ant_sizey,2*ti,freq_resolution/1e3,f0/1e6,n_antennas,itr,aar.gridu.size,rnum)
                    lines += ['kernprof -l -o {0}.lprof MOFF_FX_performance_profiling.py --layout {1} --ant-sizex {2:.1f} --ant-sizey {3:.1f} --nts {4:0d} --max-nt {5:0d} --xmax {6:.2f} --ymax {7:.2f}\n'.format(fname,layout,ant_sizex,ant_sizey,ti,itr,xmax[ind],ymax[ind])]
                elif layout.split('-')[0] == 'HEX':
                    fname = '/data3/t_nithyanandan/project_MOFF/simulated/MWA/profiling/FX_serial_all_lines/MOFF_FX_performance_comparison_{0}_{1:.1f}m_dia_{2:0d}x{3:.1f}_kHz_{4:.1f}_MHz_{5:0d}_ant_{6:0d}_acc_{7:0d}_pix_r{8:0d}'.format(layout,ant_diameter,2*ti,freq_resolution/1e3,f0/1e6,n_antennas,itr,aar.gridu.size,rnum)
                    lines += ['kernprof -l -o {0}.lprof MOFF_FX_performance_profiling.py --layout {1} --ant-diameter {2:.1f} --nts {3:0d} --max-nt {4:0d} --xmax {5:.2f} --ymax {6:.2f}\n'.format(fname,layout,ant_diameter,ti,itr,xmax[ind],ymax[ind])]
