					...
					while not self.iring.writing_ended():
						for ispan in iseq_spans:
							...
							with oseq.reserve(ogulp_size) as ospan:
								...
								fdata = BFArray(fdata, space='system')
								fdata *= phases
								...
								try:
									...
									qdata = BFArray(shape=fdata.shape, native=False, dtype='ci8', space='system')
									Quantize(fdata, qdata, scale=1./numpy.sqrt(nchan))
	...
	def main(self):
		...
		with self.oring.begin_writing() as oring:
			for iseq in self.iring.read(guarantee=True):
				...
				self.sequence_proclog.update(ihdr)
				print 'CorrelatorOp: Config - %s' % ihdr
				...
				with oring.begin_sequence(time_tag=iseq.time_tag, header=ohdr_str) as oseq:
					...
					while not self.iring.writing_ended():
						for ispan in iseq_spans:
							...
							## Setup and load
							idata = ispan.data_view(numpy.int8).reshape(ishape+(2,))
							
							## Fix the type
							tdata = BFArray(shape=ishape, dtype='ci8', native=False, buffer=idata.ctypes.data)
							
							## Copy
							udata = tdata#.copy(space='cuda')
		...
		for iseq in self.iring.read(guarantee=True):
			...
			while not self.iring.writing_ended():
				...
				for ispan in iseq_spans:
					...
					fits.write()
					fits.close()
					print 'Wrote %s to disk' % fits.filename
					intCount += 1
			...
			while not self.iring.writing_ended():
				...
				for ispan in iseq_spans:
					...
					for i in xrange(nstand):
						for j in xrange(i, nstand):
							...
							for pol in pols:
								...
								dataDict['vis'][pol].append( odata[:,j,i,p0,p1] )
					...
					for i,pol in enumerate(pols):
						...
						ax.imshow(img.image(center=(80,80)), origin='lower', interpolation='nearest')
						ax.set_title(pol)
